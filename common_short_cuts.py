...
def expand_multiple_letters(text):
    ...
    text = re.sub('[\s]+'+ PREFIX_RE + "(?P<abbreviation>WP)",
...
def test_suite():
    ...
    expanded = expand(text,sport='mlb')    
    print "text:",text,"\nexpands to:",expanded
    assert expanded == "(Average(100*((o:runs<runs) as 'W')@team and season and season=season)) as 'WP'>4 and date>PyQL.py_tools.today() as 'today'"
      
    text = "WP>4 and FD>5"
    expanded = expand(text,clients=['KS'],sport='nfl')    
    print "text:",text,"\nexpands to:",expanded
    assert expanded == "(Average(100*((o:points<points) as 'W')@team and season and season=season)) as 'WP'>4 and first downs as 'FD'>5"
