...
def game_level(**kwargs):
    ...
    dv_ret += "<BR>To plot the opponent's points vs the team's points for all Bears games in 2013 use the SDQL: "+ link(page='query_data_viz',sdql="t:points as Points, o:points as Opponent Points @ t:team=Bears and season=2013",**{'output':'plotly.scatter'})
    dv_ret += "<p><b>Histograms</b> require the SDQL syntax: <em>x @ conditions</em>. For example :"
    dv_ret += "<BR>To plot the distribution of points for all NFL games this century use the SDQL: "+ link(page='query_data_viz',sdql="t:points as Points @ 2000 <= season",**{'output':'plotly.histogram'})
    dv_ret += "<BR>To color this histogram by site, use the SDQL: "+ link(page='query_data_viz',sdql="t:points as Points @ 2000 <= season and site",**{'output':'plotly.histogram'})
    dv_ret += "<p><b>Box Plots</b> require the SDQL syntax: <em>x @ groups </em>. For example :"
    dv_ret += "<BR>See the points scored by each NFL team in 2013 use the SDQL: "+ link(page='query_data_viz',sdql="points @ team and season=2013",**{'output':'plotly.box'})
    

    num_ret += "<b>For access to the NFL game-level database</b> use the syntax: <em>game reference:parameter.</em>"
    ...
    num_ret += "<BR> To see how the Packers do after Aaron Rodgers throws more than 40 passes use the SDQL: "+ link(page='query',sdql="40 < Packers:AR:p:passes")
    num_ret += "<BR> To see how teams do when LeSean has more than 20 rushes use the SDQL: "+ link(page='query',sdql="20 < LeSean:rushes")
...
def player_level(**kwargs):
    ...
    dv_ret += "<p><b>Histograms</b> require the SDQL syntax: <em>x @ conditions</em>. For example:"
    dv_ret += "<BR>To plot the distribution of individual rushing yards for all NFL player this century use the SDQL: "+ link(page='rushing_query_data_viz',sdql="t:rushing yards as Rushing Yards @ 2000 <= season",**{'output':'plotly.histogram'})
    dv_ret += "<p><b>Box Plots</b> require the SDQL syntax: <em>x @ groups </em>. For example :"
    ...
    num_ret += "<b>For access to the NFL player-level database</b> use the syntax: <em>game reference:parameter.</em>"
    num_ret += "<BR>Where game game references are any combination of t, o, p, P, n and N (for team, opponent, previous game, previous matchup, next game and next matchup respectively). Here are a few examples: "
    ...
    num_ret += "<BR> To see players do after accounting for more than 90% of their team's rushes use the SDQL: "+ link(page='rushing_query',sdql="0.9 * Team:p:rushes < p:rushes")
    num_ret += "<BR> To find players who had a rush and a reception of over 50 yards use the SDQL: "+ link(page='rushing_query',sdql="50 < longest rush and 50 < longest reception")
    if "data_vi" in kwargs.get('form_action',''):
        return dv_ret + "<p>" + num_ret
    return num_ret + "<p>" + dv_ret
