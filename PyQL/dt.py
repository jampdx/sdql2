...
#import PyQL.dt
py_tools = PyQL.py_tools # query loop needs it like this.
...
MC_DATA_PAT = re.compile("_d\[([0-9]+)\]\[")

def optimize_mc( mc): # I don't believe in hell -mc
    # clean up None handling and
    #    replace references to _d[n] with _d_n
    mc = mc.replace("((1) or None) and ",'')
    names = ''
    dns =  list(set(MC_DATA_PAT.findall(mc)))
    for n in dns:
        names += "_d_%s = _d[%s];"%(n,n)
    mc = MC_DATA_PAT.sub(lambda g:'_d_%s['%(g.group(1),),mc)
    return mc.replace('#_names',names)

# could continue with this and organized levels of common terms.
def common_conditions(conditions):
    len_conditions = len(conditions)
    if len_conditions < 2:  # "I don't have anything in common with myself" - Kafka
        return []
    conditions = map(lambda x:x[0],conditions)
    parts = map(lambda x:PyQL.py_tools.split_not_protected(x,' and '),conditions)
    same_parts = []
...
class DT(object):
    ...
    def __init__(self,
                 data=None,
                 name=None,
                 top_metacode_lines=[],  # inside of the query loop and good for self joins
                 alias_metacode_lines=[], # outside of the loop and good for exogenous joins and other handy names
                 verbose=0,
        ...
        self.top_metacode_lines = top_metacode_lines #  joins
        self.alias_metacode_lines = alias_metacode_lines #  aliases
    ...
    def arrayize(self):
        # doesn't seem to do poo.
        for h,header in enumerate(self.headers):
            if not filter(lambda x:type(x) not in [int,float] or int(x)!=x or not -128<=x<128,self.data[h].data):
                print "casting %s as an array of type 'b'"%header
                self.data[h].data = array.array('b',self.data[h].data)
            if not filter(lambda x:type(x) not in [int,float] or int(x)!=x or not -32786<=x<32768,self.data[h].data):
                print "casting %s as an array of type 'h'"%header
                self.data[h].data = array.array('h',self.data[h].data)
    ...
    def del_column(self,index,rebuild_header_dict=1):
        ...
        if rebuild_header_dict: self.build_header_dict()
    ...
    def build_lexer(self,parameters=[],db_string_pat=''):
        ...
        #print "building new lexer!"
        parameters = map(lambda x:x+"(?!(?:[a-zA-Z_]|[\s]*\())",parameters) # 20130806: add negative look ahead so parameters don't match part of a string. # 20171016: was (?![a-zA-Z_]), add also not starting a function
        ...
        for tok_terms in condition_tok_terms: # and|or delimted
            ...
            for t in range(len(tok_terms)):
                ...
                for tok in toks:
                    ...
                    elif t_type == "WORD":                   #20120714: bears=>bears; Bears=>'Bears'
    ...
    def build_metacode(self,fields,conditions,
                       field_cache,condition_cache,top_metacode_lines,alias_metacode_lines): #bmc

        mc = '''def _query(self,_d,_result,_field_cache,_condition_cache):\n #_names\n %s\n for _i in xrange(%d):\n'''%(';'.join(alias_metacode_lines), len(self.data[0].data), )
        #c += r'\tglobal exc_count\n'
        #mc += "\tprint '_i=',_i\n"
    ...
    def query(self, pyql, default_fields = None, indices=None, parse_only=0):
        #print "dt.query gets",pyql
        global exc_count
        ...
        if parse_only: return 'no query indices'
        ...
        self.build_metacode(self.fields,self.conditions,
                            ...
                            self.top_metacode_lines,self.alias_metacode_lines)

        self.metacode = optimize_mc(self.metacode)
        ...
        # create the '_query' function in the local scope
        ...
        exc_count = 0
        lol_data = [col.data for col in self.data]
        _query(self,lol_data,result,field_cache,condition_cache)
    ...
    def sort_by_column(self,offset=0,compare=cmp):
        #this doesn't change the order when accesses as dt[col][i]" need dt[col].data[i] "
        ...
        for c in self.data:
            td.append(c.data)
        tr = PyQL.py_tools.transpose_table(td)
        ...
        if type(tr[0][offset]) is type('abc') or (
            ...
            #print 'sorign by value'
            tr.sort(lambda x,y,offset=offset,compare=compare:compare(x[offset],y[offset]))
        #elif type(tr[0][offset]) is list and 0<len(tr[0][offset]):
            #print 'sorign by last item in list'
        #    tr.sort(lambda x,y,offset=offset,compare=compare:compare(x[offset][-1],y[offset][-1])) # cmp the last value of list
        else:
            #print 'reversing by value: type',type(tr[0][offset])
        ...
        for c in range(len(self.data)):
            ...
            self[c].data = tr[c]
            #print "setting data for sorted",self.headers[c],self.data[c].data[:100]
    ...
    def value(self,header,default=None):
        ...
        for head in all_combinations(headers):
            ...
            if type(v[0].value()) in NUMBER_TYPES:
                ...
                summ =  v[0].value()
            count += 1
        # 20170119 for no key, continue to return None for Average, return 0 for Sum:20171026 abandon this idea
        if isinstance(self[0][0],PyQL.columns.Average):
            if count: return 1. * summ / count # count is the number of square bracket keys
            else: return None
        # 20171013 for no key and not N return 0 for Sum, if N<count return None
        #if isinstance(self[0][0],PyQL.columns.Sum) and not self[0][0].N:
        #    return summ or 0 # if it never happened and N is not set, let None be 0
        return summ
...
def dt_from_file(file,
    ...
    for i in range(len(headers)):
        ...
        #print "format:",format
        dt.add_object(PyQL.columns.Column(name=headers[i],data=list(data[i]),format=format))
    dt.build_lexer()
    return dt
...
def test_nest():
    ...
    c3 = PyQL.columns.Column(name="first name",format="%s")
    c4 = PyQL.columns.Column(name="age",format="%s")
    c5 = PyQL.columns.Column(name="cdt",format="%s")
    dt = DT(data = [c1,c2,c3,c4,c5],name="test table")

    c5b1 = PyQL.columns.Column(name="height",format="""lambda h:"%0.0f'%0.0f''"%(int(h)/12,int(h)-12*(int(h)/12))""")

    c5b2 = PyQL.columns.Column(name="weight",format="""lambda w:'%d\#'%w""")
    c5b3 = PyQL.columns.Column(name="first name",format="%s")
    c5b4 = PyQL.columns.Column(name="age",format="%s")

    c5b = DT(data = [c5b1,c5b2,c5b3,c5b4],name="test child table")
    c5b1.append(40);c5b2.append(100);c5b3.append('Jr');c5b4.append(8)

    c5m1 = PyQL.columns.Column(name="height",format="""lambda h:"%0.0f'%0.0f''"%(int(h)/12,int(h)-12*(int(h)/12))""")

    c5m2 = PyQL.columns.Column(name="weight",format="""lambda w:'%d\#'%w""")
    c5m3 = PyQL.columns.Column(name="first name",format="%s")
    c5m4 = PyQL.columns.Column(name="age",format="%s")
    c5m = DT(data = [c5m1,c5m2,c5m3,c5m4],name="test child table2")
    c5m1.append(35);c5m2.append(90);c5m3.append('MJr');c5m4.append(7)
    c5m1.append(30);c5m2.append(80);c5m3.append('Mikey');c5m4.append(6)

    c1.append(65);c2.append(180);c3.append('Bob');c4.append(23);c5.append(c5b)
    c1.append(69);c2.append(150);c3.append("mike");c4.append(43);c5.append(c5m)
...
def test_suite(dt):
    ...
    dt.show_metacode=1
    pyql = "Sum(age@Sum(weight)>200)@1"
    print "pyql:",pyql
    res = dt.query(pyql)
    assert res[0][0].value() == 72

    pyql = "age,height,first name@first name<0"
    print "pyql:",pyql
    res = dt.query(pyql)
    print "res:",res
    #assert res[0][0].value() == 72
    #print "short cut the test suite"
    #return
    ...
    pyql = "Average(height)@Sum(age@Sum(weight@2)[2]>10)[Sum(weight@7)[7]>10]>60 and height"
    print "pyql:",pyql
    res = dt.query(pyql)
    print res
...
def time_nfl(nfl):
    ...
    queries = ['date,team@A(points@team and passing yards and rushing yards)>30 and S(points@team and passing yards and rushing yards)>30',
               'R($1),R($2),R($3),R($4),R($5),R($6),R($7),R($8),R($9),R($10),R($11),R($12),R($13),R($14),R($15),R($16),R($17),R($18),R($19),R($20),R($21),R($22),R($23),R($24),R($25),R($26),R($27),R($28),R($29),R($30),R($31),R($32),R($33),R($34),R($35),R($36),R($37),R($38),R($39),R($40),R($41),R($42),R($43),R($44),R($45),R($46),R($47),R($48),R($49),R($50),R($51),R($52),R($53),R($54),R($55),R($56),R($57),R($58),R($59)@1']

    for c, query in enumerate(queries, 1):
        print "query %d:"%c,
        sys.stdout.flush()
        data = []
        ...
        print "min=%.2f avg=%.2f max=%.2f σ=%.2f" % (
            min(data), M, max(data), S )
...
def test_sort():
    dt = test_dt()
    dt = dt.query('S(weight),S(age)@first name')
    dt = dt.reduced(None)
    print dt.html()
    dt.sort_by_column(0)
    print "sorted"
